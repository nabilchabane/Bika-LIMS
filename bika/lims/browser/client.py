...
class ClientStorageLocationsView(BikaListingView):
    """This is displayed in the "Storage Locations" tab on each client
    """

    def __init__(self, context, request):
        super(ClientStorageLocationsView, self).__init__(context, request)
        self.catalog = "bika_setup_catalog"
        self.contentFilter = {
    ...
    def __call__(self):
        mtool = getToolByName(self.context, 'portal_membership')
        checkPermission = mtool.checkPermission
        if checkPermission(AddStorageLocation, self.context):
            self.context_actions[_('Add')] = \
                {'url': 'createObject?type_name=StorageLocation',
                 'icon': '++resource++bika.lims.images/add.png'}
        return super(ClientStorageLocationsView, self).__call__()
